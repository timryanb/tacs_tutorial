...
bdfFile = os.path.join(os.path.dirname(__file__), '../input files/battery_pack.bdf')
...
def elemCallBack(dvNum, compID, compDescript, elemDescripts, globalDVs, **kwargs):
    ...
    con = constitutive.PlaneStressConstitutive(prop, t=tplate)
    ...
    for elemDescript in elemDescripts:
        if elemDescript == 'CQUAD4':
            ...
            raise ValueError(f"Element '{elemDescript}' not recognized")
...
transientProblem = FEAAssembler.createTransientProblem('Runaway', tInit=0.0, tFinal=20.0, numSteps=100)
...
compIDs_01 = FEAAssembler.selectCompIDs(["Battery.01"])  # adjacent battery
